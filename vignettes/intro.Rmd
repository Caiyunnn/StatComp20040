...
vignette: >
  %\VignetteIndexEntry{StatComp20040}
...
## bootstrapCI
The source R code for bootstrapCI is as follows:
```{r,eval=FALSE}
...
the example was given as follows:
```{r,eval=FALSE}
bootstrapCI(faithful$eruptions,10000,0.05,border = TRUE,col="blue")

```

## the k-nearest neighbor density estimation
...
when RJ test statistic $<$ marginal value, we reject the null hypothesis of normality,
vice versa. above examples get the right test results.
